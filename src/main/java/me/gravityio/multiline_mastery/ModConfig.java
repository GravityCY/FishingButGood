...
public class ModConfig {
    ...
    public static final ConfigClassHandler<ModConfig> HANDLER = ConfigClassHandler.createBuilder(ModConfig.class)
            .id(MultilineMasteryMod.id("my_config"))
    ...
    public Screen getScreen(Screen parent) {
        return YetAnotherConfigLib.create(HANDLER, (defaults, config, builder) -> {
            ...
            category.name(Component.translatable("yacl.multilinemastery.title"));

            var angleOpt = Option.<Integer>createBuilder()
                    .name(Component.translatable("yacl.multilinemastery.angle.label"))
                    .description(OptionDescription.of(Component.translatable("yacl.multilinemastery.angle.description")))
                    .binding(defaults.angle.get(), config.angle::get, config.angle::set)
                    .controller(opt -> IntegerSliderControllerBuilder.create(opt).step(1).range(1, 90).formatValue(v -> Component.literal(v + "deg")));

            var highlightOpt = Option.<Boolean>createBuilder()
                    .name(Component.translatable("yacl.multilinemastery.highlight.label"))
                    .description(OptionDescription.of(Component.translatable("yacl.multilinemastery.highlight.description")))
            ...
            builder.title(Component.translatable("yacl.multilinemastery.title"));
